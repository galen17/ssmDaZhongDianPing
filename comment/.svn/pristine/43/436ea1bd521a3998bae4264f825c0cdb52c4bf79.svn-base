...
public class AdServiceImpl implements AdService {
	...
	public boolean add(AdDto adDto) {
		...
		if (adDto.getImgFile() != null && adDto.getImgFile().getSize() > 0) {//校验是否有文件
			String fileName = System.currentTimeMillis() + "_" + adDto.getImgFile().getOriginalFilename();
			File file = new File(adImageSavePath + fileName);
			File fileFolder = new File(adImageSavePath);
			if (!fileFolder.exists()) {//目录不存在，多级创建
				fileFolder.mkdirs();
			}
			try {
				adDto.getImgFile().transferTo(file);//spring提供的保存图片方法
	...
	public boolean modify(AdDto adDto) {
		...
		//最后删除原图片，避免删了原图片，新图片每新增成功。
		//这里发生错误不会影响整个修改，只是没删掉。可以加个定时方法，定时检查没删除成功的，进行删除。
		if (fileName != null) {
